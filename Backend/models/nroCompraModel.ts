...
const NroCompraModel = {
  ...
  obtenerTodos: (): Promise<NroCompra[]> => {
    const sql =
      "SELECT id, id_proveedor, fecha AS fecha_compra, total FROM nro_compra ORDER BY id ASC";
  ...
  obtenerPorId: (id: number): Promise<NroCompra> => {
    const sql =
      "SELECT id, id_proveedor, fecha AS fecha_compra, total FROM nro_compra WHERE id = $1";
  ...
  crear: (
    ...
    const sql =
      "INSERT INTO nro_compra(id_proveedor, fecha, total) VALUES($1, $2, $3) RETURNING id";
  ...
  actualizar: (
    ...
    const sql =
      "UPDATE nro_compra SET id_proveedor = $1, fecha = $2, total = $3 WHERE id = $4";
    return dao.actualizar(sql, [id_proveedor, fecha_compra, total, id]);
  },
