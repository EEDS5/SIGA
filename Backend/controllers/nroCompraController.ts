...
export const mostrarNroCompra = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const actualizarNroCompra = async (req: Request, res: Response): Promise<void> => {
    const id = parseInt(req.params.id, 10); // Convertir id a número
    const { proveedor_id, fecha_compra, total } = req.body;

    try {
        await NroCompra.actualizar(id, proveedor_id, fecha_compra, total);
...
export const eliminarNroCompra = async (req: Request, res: Response): Promise<void> => {
    const id = parseInt(req.params.id, 10); // Convertir id a número
